...
const PurchaseAndBilling = (props) => {

    const [loader, setLoader] = useState(false)
    const [header, setHeader] = useState('Purchase & Billing');
    ...
    useEffect(() => {
        setLoader(true)
        getAMCLogData(propertyId, setAmcLogAudit, tokenId, userId, "4", filterMonth).then(function (obj) {
    ...
    const updateData = async () => {
        ...
        try {
            ...
            if (res.status === 204) {
                ...
                resetVal()
                getAMCLogData(propertyId, setAmcLogAudit, tokenId, userId, "4", filterMonth);
        ...
        <SafeAreaView style={{ height: "92%", backgroundColor: "#fff" }}>
            ...
            <View style={bg ? { backgroundColor: "#000000bf", opacity: 0.75 } : { backgroundColor: "#fff" }}>
                <View style={{ backgroundColor: "#2D9cDB", height: 80, justifyContent: "space-between", flexDirection: "row", paddingHorizontal: 20, alignContent: "center", paddingTop: 35 }}>
                    <Text style={{ color: "#fff", fontSize: 20, lineHeight: 27, fontWeight: "700", width: 200 }}>Purchase & Billing</Text>
                    {/* <TouchableOpacity onPress={() => console.log("asd")}>
...
export default PurchaseAndBilling
