...
import { InventoryAudit, RequestInventoryForm, SendInventory } from "../Components/Audit";
import { showMessage, hideMessage } from "react-native-flash-message";
var isBottomSheet = false;

const SendInventoryAudit = (props) => {
    ...
    const updateData = async () => {
        ...
        try {
            ...
            if (res.status === 204) {
                ...
                getInventoryAuditList(setInventoryAuditList, user.accessToken, user.userData.id, propertyId, filterMonth, false, true)
    ...
    useEffect(() => {
        if (user.permission_list != null) {
            if (user.permission_list[0].send_inventory_audit) {
                getInventoryAuditList(setInventoryAuditList, user.accessToken, user.userData.id, propertyId, filterMonth, false, true, true).then(function (obj) {
                    console.log("HERE end ", obj);
                    setLoader(false)
                })

                // getInventoryAuditListNoti(setInventoryAuditList, user.accessToken, user.userData.id, user.userData.firstName, true,true)

            } else {
                getInventoryAuditList(setInventoryAuditList, user.accessToken, user.userData.id, propertyId, filterMonth, false, true, false).then(function (obj) {
                    console.log("HERE end ", obj);
                    setLoader(false)
                })
    ...
    const renderContent = () => (
        ...
        openbottomsheet === "audit" ?
            <SendInventory
                ...
                verityStatus={user.permission_list[0].send_inventory_audit}
            />
            :
                ...
                <View
                    ...
                    <View style={{
                        ...
                        <Text style={{
                            ...
                            {/* Pending */}
                            {
                                item.status === "0" ? "On the way" : item.status === "1" ? "Delivered" : item.status === "2" ? "Delivered with difference" : "Pending"
                            }
                        </Text>
                    </View>
        ...
        <SafeAreaView style={{ height: "92%", backgroundColor: "#fff" }}>
            ...
            <View style={bg ? { backgroundColor: "#000000bf", opacity: 0.75 } : { backgroundColor: "#fff" }}>
                <View style={{ backgroundColor: "#2D9cDB", height: 80, justifyContent: "space-between", flexDirection: "row", paddingHorizontal: 20, alignContent: "center", paddingTop: 35 }}>
                    <Text style={{ color: "#fff", fontSize: 20, lineHeight: 27, fontWeight: "700", }}>Send Inventory Audit</Text>
                    <TouchableOpacity onPress={() => console.log("asd")}>
            ...
            { user.permission_list != null ? !user.permission_list[0].send_inventory_audit ?
                <TouchableOpacity
                    onPress={() => {
                        console.log("asdasS");
                        Setopenbottomsheet('audit')
...
export default SendInventoryAudit;
