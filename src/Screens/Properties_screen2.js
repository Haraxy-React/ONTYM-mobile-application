...
const Properties_screen1 = (props, { navigation }) => {
  ...
  const staticData = [
    ...
    },
    {
      ...
      screen_name: "purchase_and_billing"
    },
  ...
  return (
    <ScrollView style={{ flex: 1, marginBottom: 60, backgroundColor: "#fff" }}>
      ...
      <Image resizeMode="contain" style={{ width: "95%", height: 235, alignSelf: "center", marginTop: 10 }} source={{
        uri: item.property_image

      }} />
      <Text style={{ fontSize: 24, fontWeight: "600", lineHeight: 36, fontFamily: "Poppins-SemiBold", color: "#2d9cdb", paddingLeft: "5%" }}>{item.property_name}</Text>
      <Text style={{ fontSize: 18, fontWeight: "400", lineHeight: 27, fontFamily: "Poppins-Regular", color: "#979797", paddingLeft: "5%" }}>{item.location}</Text>
      ...
      <View style={{ borderTopWidth: 2, width: "95%", alignSelf: "center", borderColor: "#c4c4c4" }}>
        {staticData.map((item) => (
          <TouchableOpacity onPress={item.screen_name === "null" ? () => { console.log('nulll screen'); } : () => { props.navigation.navigate(item.screen_name, { property_id: propertyId }) }} style={{ justifyContent: "space-between", margin: 10, borderBottomWidth: 1, borderColor: "#d9d9d9", width: "95%", flexDirection: "row", padding: 5 }} >
